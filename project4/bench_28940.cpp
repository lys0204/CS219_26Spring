// Local benchmark for OpenCV issue #28940.
// This is not intended for the final PR; it compares original vs fixed builds.
...
static double runOnce(int iterations, const std::vector<std::vector<cv::Point> >& contours, double& checksum)
{
    ...
    for (int i = 0; i < iterations; ++i)
    {
        ...
        cv::drawContours(image, contours, -1, cv::Scalar(0, 255, 0),
                         2, cv::LINE_8, cv::noArray(), INT_MAX, cv::Point(0, 0));
    }
